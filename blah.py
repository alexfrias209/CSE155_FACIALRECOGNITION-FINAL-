...
def sending(name, user_profile):
    ...
    def sendText(mes, numbers):
        for num in numbers:
            try:
                ...
                print(f"Failed to send message to {num}. Error: {e.msg}")
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix='.mp3') as temp_audio_file:
        output.save(temp_audio_file.name)
...
def create_embeddings(user_profile, data_path):
    ...
    if user_profile.data_file:
        default_storage.delete(user_profile.data_file.path)
...
def facial(user_profile,video_path, video_session):
    ...
    def process_video_face_recognition(video_path, data, threshold=0.8, exit_count=15, video_session=None):
